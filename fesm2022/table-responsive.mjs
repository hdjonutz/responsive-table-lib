...
import { Directive, TemplateRef, Input, ContentChild, Injectable, EventEmitter, Output, Component, ChangeDetectionStrategy, ViewChild, ContentChildren, Pipe, NgModule } from '@angular/core';
...
class TableHeaderComponent {
    ...
    changeTo(nameAction, key, type) {
        Object.keys(this.selectToAction).forEach((s) => this.selectToAction[s] = IStates.Nothing);
...
class ReactiveTableRowCellComponent {
    constructor(tableBody, rowService, cdr) {
        this.rowService = rowService;
        this.cdr = cdr;
    ...
    ngOnInit() {
        this.rowsObs = this.rowService.getChanges().subscribe((res) => {
            this.rowsExpanded = res;
        });
        // timer(500).subscribe(() => {
    ...
    getRowHeight(row) {
        const styleRow = {
            ...
            // 'min-height': '160px',  // ← default
            // 'max-height': '160px',  // ← default
            // 'overflow': 'hidden'
        };
    ...
    get rowHeight() {
        return this.config?.rowHeight || this.config?.rowMinHeight || 40;
    }
    getCellWidth(att) {
        const styleCell = {};
...
class TableResponsiveComponent {
    ...
    resetFiltersAndSortColumnFilterInput(sort) {
        this.sortHeader = [];
        this.sortColumnFilterInput(sort);
    }
    ;
    sortColumnFilterInput(sort) {
        ...
        const foundIdx = this.sortHeader.findIndex((s) => s.key === sort.key);
        if (foundIdx >= 0) {
            this.sortHeader[foundIdx] = { key: sort.key, direction: sort.direction, type: sort.type };
        }
        else {
            this.sortHeader.push({ key: sort.key, direction: sort.direction, type: sort.type });
        }
        this.sortHeader = this.sortHeader.filter((s) => s.direction !== IStates.Nothing) || [];
        // last changed -> last filter has smaller priority
        const sortByAllKeys = (a, b, sortHeader) => {
            for (let i = 0; i < sortHeader.length; i++) {
                const { key, type, direction } = sortHeader[i];
                // Define sorting direction
                const d = direction === IStates.Nothing ? 0 : direction === IStates.BiggerToSmaller ? -1 : 1;
                ...
                if (type === 'number') {
                    ...
                    // @ts-ignore
                    res = (a[key] + '').localeCompare(b[key] + '', 'de', { numeric: type === 'number', sensitivity: "case" });
                }
                // If there's a difference, return the sorted result
                if (res !== 0) {
                    return res * d;
                }
            }
        ...
        if (this.sortHeader.length > 0) {
            this.tabledata = this.tabledata.sort((a, b) => sortByAllKeys(a, b, this.sortHeader));
        }
    }
